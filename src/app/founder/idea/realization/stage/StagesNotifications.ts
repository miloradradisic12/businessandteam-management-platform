...
export class StageNotifications {
  ...
  chaeckStageCompletion(newStage:any, newStageIndex: number): Observable<any> {
    ...
    let activeStage = this.stateStorage.getStageState(this.activeStageName, this.idea);
    let prevStage = this.stateStorage.getStageState(prevStageName, this.idea);
    let newStageObj = this.stateStorage.getStageState(newStage.url, this.idea);
