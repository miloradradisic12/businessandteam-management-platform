...
export class FounderProjectsOverviewComponent implements OnInit {
  ...
  openNotarize(project) {
    ...
    this.projectsService.getNotarizeResponse(project.id).subscribe((obj) => {
      if (obj && obj.transaction_id && obj.transaction_id != null && obj.transaction_id != '') {
  ...
  openSummary(project) {
    if (project.progress === 100) {
      ...
      this.router.navigate(['/founder/idea', 'realization', { idea: project.id }], { relativeTo: this.route });
    } else {
