...
@Component({
    ...
    <select [(ngModel)]="settings.fontSize" (ngModelChange)="onChange()" class="form-control" >
        <option class='option' *ngFor='let option of fontSizes' [ngValue]="option.value">{{option.text}}</option>
    </select>
    <select [(ngModel)]="settings.fontFamily" (ngModelChange)="onChange()" class="form-control">
