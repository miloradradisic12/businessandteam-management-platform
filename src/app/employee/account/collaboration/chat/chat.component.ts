...
export class DocumentChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  sendMessage(messageType?: string) {
    if (this.chatRoom) {
      if (this.messageText != '' && this.messageType != '') {
        ...
        // else if(messageType == 'opinion' || messageType == 'decisions' || messageType == 'argument' || messageType == 'gut_feeling') {
        //   debugger;
        //   this.postUploadAttachment();
        // }
        else {
          this.sendMsg();
        }
  ...
  postUploadAttachment() {
    ...
    this.chatService.postUploadAttachment(this.messageType, 
      this.messageText, this.sourceAttachment, this.chatRoom.room_id).flatMap(() => {
      return this.chatService.getChatHistory(this.chatRoom.room_id);
    }).subscribe((messages: ChatMessageModel[]) => {
      this.resetValues();
      this.messageType = '';
